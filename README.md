...
| 公开路径 | 所属仓库 | 本仓库职责 |
| --- | --- | --- |
| `/` | `ElmerH313/ElmerH313.github.io` | 品牌头部、入口卡片、语言切换和页脚 |
...
- Elmer.H 品牌标题与 Product Hub 标识。
- GuitarMetro 和 Fingerboard Pro 两张统一入口卡片。
- 无手动选择时按 IP 国家代码自动显示中文或英文，识别失败回退浏览器语言；手动选择保存到 `localStorage('lang')` 并始终优先。
- GitHub、Email、根隐私政策和版权信息。

主页不提供个人介绍、社交二维码、留言板、产品截图、功能清单、下载按钮或产品手册内容。入口图标是主页自有的标准化展示副本，产品运行时不从本仓库读取资源。
